...
def try_convert_dates(df):
    ...
    for col in converted.columns:
        ...
        looks_like_date_name = any(k in col_lower for k in ["date", "날짜", "일시", "연월", "year", "년도", "월"])
...
def make_data_profile(df):
    ...
    for col in numeric_cols:
        ...
        if count > 0:
            outlier_info.append({"column": col, "outliers": count})
...
def dataframe_summary_text(df, max_rows=8):
    ...
    numeric_desc = df[profile["numeric_columns"]].describe().round(3).to_dict() if profile["numeric_columns"] else {}
...
def draw_chart(df, chart_type, x_col=None, y_col=None, hue_col=None):
    ...
    if chart_type == "선 그래프" and x_col and y_col:
        ...
        sns.scatterplot(data=plot_df, x=x_col, y=y_col, hue=hue_col if hue_col else None, ax=ax)
        ...
        if numeric_df.shape[1] >= 2:
            ...
            ax.text(0.5, 0.5, "숫자형 열이 2개 이상 필요합니다.", ha="center", va="center")
        ...
        ax.text(0.5, 0.5, "차트를 그리기 위한 열을 선택해주세요.", ha="center", va="center")
...
with st.sidebar:
    ...
    api_key = st.text_input("OpenAI API Key", type="password", label_visibility="collapsed")
    ...
    max_preview_rows = st.slider("미리보기 행 수", 5, 30, 10)
    auto_clean = st.checkbox("열 이름 공백 자동 정리", value=True)
    convert_dates = st.checkbox("날짜형 열 자동 인식", value=True)
...
# 메인 화면
# -----------------------------
st.markdown('<div class="main-title">Streamlit — 기후 데이터 AI 분석 도우미</div>', unsafe_allow_html=True)
st.markdown(
...
selected_question = st.radio("추천 질문 중 하나를 선택하거나 아래에 직접 입력하세요.", recommended_questions)
custom_question = st.text_area("직접 질문 입력", placeholder="예: 이 데이터에서 기후 변화의 장기 추세를 설명할 수 있나요?")
...
chart_options = ["선 그래프", "막대그래프", "산점도", "히트맵", "박스플롯"]
chart_type = chart_col1.selectbox("차트 종류", chart_options)

all_cols = df.columns.tolist()
def safe_index(options, preferred):
    if preferred and preferred in options:
        return options.index(preferred)
    return 0

x_default = date_cols[0] if date_cols else (category_cols[0] if category_cols else (all_cols[0] if all_cols else None))
y_default = numeric_cols[0] if numeric_cols else (all_cols[0] if all_cols else None)

x_col = chart_col2.selectbox("X축", [None] + all_cols, index=([None] + all_cols).index(x_default) if x_default in all_cols else 0)
y_col = chart_col3.selectbox("Y축", [None] + numeric_cols, index=([None] + numeric_cols).index(y_default) if y_default in numeric_cols else 0)
hue_col = chart_col4.selectbox("색상 구분", [None] + category_cols, index=0)

fig = draw_chart(df, chart_type, x_col=x_col, y_col=y_col, hue_col=hue_col)
st.pyplot(fig)
...
5. 추가 탐구 질문 3개
"""
        ai_result = ask_ai(api_key, model, system_prompt, user_prompt)
...
problem_type = st.selectbox("문제 유형", ["자동채점형", "사고력 평가형", "객관식", "세특 작성용 탐구 질문"])
...
- 한국어로 작성할 것
"""
        problems = ask_ai(api_key, model, system_prompt, user_prompt)
...
ai_result_for_report = st.session_state.get("ai_result", "아직 AI 분석을 실행하지 않았습니다.")
report_md = make_markdown_report(df, question, ai_result_for_report)
